...
public class AnnotationItem {
    ...
    public static AnnotationItem read(RandomInput in, ReadContext rc) {
        AnnotationItem out = new AnnotationItem();
        out.visibility = in.readByte();
        out.annotation = EncodedAnnotation.read(in, rc);
        return out;
    }

    public static AnnotationItem[] readSet(RandomInput in, ReadContext rc) {
        int size = in.readInt();
        AnnotationItem[] out = new AnnotationItem[size];
        for (int i = 0; i < size; i++) {
            out[i] = AnnotationItem.read(in.duplicate(in.readInt()), rc);
        }
        return out;
    }

    public static AnnotationItem[][] readSetList(RandomInput in, ReadContext rc) {
        int size = in.readInt();
        AnnotationItem[][] out = new AnnotationItem[size][];
        for (int i = 0; i < size; i++) {
            ...
            if (annotations_off != 0) {
                out[i] = AnnotationItem.readSet(
                        in.duplicate(annotations_off), rc);
            } else {
